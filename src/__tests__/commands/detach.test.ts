...
describe("detachSession", () => {
  ...
  it("updates all running session statuses when detaching all", async () => { mockStore.listSessions.mockResolvedValue([makeRecord({ sessionName: "s1", status: "running" }), makeRecord({ sessionName: "s2", status: "running" }), makeRecord({ sessionName: "s3", status: "complete" })]); await detachSession(mockStore, { all: true }); expect(mockStore.updateStatus).toHaveBeenCalledWith("s1", "running"); expect(mockStore.updateStatus).toHaveBeenCalledWith("s2", "running"); });
